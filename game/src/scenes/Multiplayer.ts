...
export default class Multiplayer extends Phaser.Scene {
	...
	private spawnOtherPlayer(data: webRTCData) {
		new Promise((resolve) => {
			...
			loader.image(data.texture.ship.key, data.texture.ship.path);
			loader.image(data.texture.laser.key, data.texture.laser.path)

			loader.once(Phaser.Loader.Events.COMPLETE, () => {
				let gameScene = this.scene.get(SceneKeys.Game)
				this.otherShip[`${data.id}`] = gameScene.add.playerShip(data.position.x, data.position.y, data.texture.ship.key)
